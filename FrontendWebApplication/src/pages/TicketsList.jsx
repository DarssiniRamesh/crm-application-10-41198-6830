...
export default function TicketsList() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setErr(null);
    api.get('/tickets', { params: { page, pageSize, sort, filter } })
  ...
  return (
    <section aria-labelledby="tickets-heading">
      ...
      <div className="filters" role="region" aria-label="Filters">
        <label>
          Search:
          <input value={filter} onChange={(e) => updateParam('filter', e.target.value)} aria-label="Filter tickets" />
        </label>
        <label>
          Sort:
          <select value={sort} onChange={(e) => updateParam('sort', e.target.value)} aria-label="Sort tickets">
            ...
            <tbody>
              {items.map((t) => (
                <tr key={t.id}>
                  ...
                  <td>{t.assignedAgent?.username || ''}</td>
                  <td>{t.createdAt ? new Date(t.createdAt).toLocaleString() : ''}</td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan="10">No tickets found.</td></tr>}
            </tbody>
          </table>
